...
class Dnsmasq(DhcpLocalProcess):
    ...
    def should_enable_metadata(cls, conf, network):
        ...
        # eBay customization:
        # We do not rely on l3 agent, and it is also weird to create a metadata
        # subnet for each existing network. So we add the option
        # (enable_isolated_metadata_nocheck) to enable metadata-proxy regardless
        # of whether it is isolated.

        if conf.enable_isolated_metadata_nocheck:
            return True
