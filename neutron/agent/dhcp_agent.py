...
class DhcpAgent(manager.Manager):
    OPTS = [
        ...
        cfg.BoolOpt('enable_isolated_metadata_nocheck', default=False,
                    help=_("When enable_isolated_metadata is True, enable "
                           "metadata-proxy regardless of whether the network"
                           " is isolated.")),
        ...
        cfg.StrOpt('metadata_proxy_socket',
                   ...
                   help=_('Location of Metadata Proxy UNIX domain '
                          'socket')),
        cfg.IntOpt('num_subprocess_failure', default=2,
                   help=_("Number of continuous failures of sub-processes "
                          "before triggering resync."))
    ]
    ...
    def __init__(self, host=None):
        ...
        self.subprocess_failure_count = 0
    ...
    # ebay customization start {{
    def get_driver_property(self, property, network):
        """Get a property on a DHCP driver instance."""
        try:
            # the Driver expects something that is duck typed similar to
            # the base models.
            driver = self.dhcp_driver_cls(self.conf,
            ...
            property = getattr(driver, property)
            return property
            ...
            if (isinstance(e, common.RemoteError)
                ...
                LOG.exception(_('Unable to get %(property)s dhcp for %(net_id)s.')
                              % {'net_id': network.id, 'property': property})

    def subprocess_resync_confirm(self, need_resync):
        """Confirm resync only at num_subprocess_failure continuous failures"""
        if not need_resync:
            self.subprocess_failure_count = 0
            return False

        self.subprocess_failure_count += 1
        if self.subprocess_failure_count < \
                self.conf.num_subprocess_failure:
            return False

        return True

    def needs_resync_subprocesses(self):
        ...
        for network_id in self.cache.get_network_ids():
            ...
            if self.dhcp_driver_cls.should_enable_metadata(
                self.conf, network):
        ...
        if self.subprocess_resync_confirm(need_resync):
            LOG.warn(_('Continuous failure confirmed, will resync'))
            return True

        return False
    # ebay customization end }}
    ...
    def _periodic_resync_helper(self):
        ...
        while True:
            ...
            if self.needs_resync_reasons or self.needs_resync_subprocesses():
