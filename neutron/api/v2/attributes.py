...
def _validate_string_starting_with_alphabet(data, valid_values=None):
    try:
    ...
    msg = _("'%s' name must start with an alphabet") % data
    LOG.debug(msg)
    return msg

def _validate_string_length(data, valid_values=None):
    try:
        if len(data) < int(valid_values):
            return
    except TypeError:
        pass

    msg = ("'%(data)s' name should be less than %(length)s charecters") % {
        'data': data, 'length': valid_values}
    LOG.debug(msg)
    return msg
...
validators = {'type:dict': _validate_dict,
              ...
              'type:string_starting_with_alphabet': _validate_string_starting_with_alphabet,
              'type:string_length': _validate_string_length}
