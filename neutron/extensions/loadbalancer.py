...
class VIPNameAndPoolNameCannotBeSame(qexception.NeutronException):
    message = _("%(name)s vip name cannot be same as pool name")
...
class NameHasSpecialChars(qexception.NeutronException):
    message = _("Name %(name)s cannot have special characters. Allowed char "
                "set is [a-zA-Z0-9_-.] for all resources with '_' excluded for "
                "VIPs.")
...
class DuplicateVIPRequestWithSameIPAndPort(qexception.NeutronException):
    message = _("Another VIP %(vipname)s already exists with the same ip and port")
...
class CouldNotDeleteTheLastMemberofPoolWhenVIPAdminStateUP(qexception.NotFound):
    message = _("Couldn't delete the last member of the pool when the vip admin state is up")
...
class VPCQuotaExceeded(qexception.NeutronException):
    message = _("%(entity)s quota exceeded for %(vpc)s vpc : Current usage is "
                                "%(in_use)s, allowed quota is %(allowed_quota)s")

class NotImplementedError(qexception.NeutronException):
    message = _('')
...
RESOURCE_ATTRIBUTE_MAP = {
    VIPS: {
        ...
        'name': {'allow_post': True, 'allow_put': True,
                 'validate': {'type:string': None,
                              'type:string_starting_with_alphabet': None,
                              'type:string_length': 127},
                 'default': '',
                 'is_visible': True},
    ...
    POOLS: {
        ...
        'label': {'allow_post': True, 'allow_put': False,
                     'validate': {'type:string': None},
                     'is_visible': True, 'default': ''},
        ...
        'status_description': {'allow_post': False, 'allow_put': False,
                               'is_visible': True},
        'extra': {'allow_post': True, 'allow_put': True,
                  'is_visible': True, 'default': None},
        'tenant_vpc': {'allow_post' : True, 'allow_put' : False,
                'validate': {'type:string': None}, 'is_visible': True, 'default' : ''}
    },
    MEMBERS: {
        'id': {'allow_post': False, 'allow_put': False,
               'validate': {'type:uuid': None},
