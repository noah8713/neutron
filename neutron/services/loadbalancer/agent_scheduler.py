...
class LbaasAgentSchedulerDbMixin(agentschedulers_db.AgentSchedulerDbMixin,
    ...
    def get_lbaas_agents(self, context, active=None, filters=None, sorted = None):
    ...
    # This method get pools from poolloadbalanceragentbinding table based on filter.
    # Filters is a map with key as the column and values can be a single value or multiple
    # Also pass column for sorting and its order(asc or dsc)
    def list_pools_on_pool_agent_binding(self, context, filters = None, sorted = None):
        query = context.session.query(PoolLoadbalancerAgentBinding.pool_id)
        if (sorted is not None):
            query = query.order_by(sorted)
        ...
        if filters:
            for key, value in filters.iteritems():
                column = getattr(PoolLoadbalancerAgentBinding, key, None)
                if column:
                    query = query.filter(column.in_(value))
