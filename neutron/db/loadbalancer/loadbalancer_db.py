...
import lb_notifier as notifier
...
class HasTimeStamp(models.TimestampMixin):
    created_at = sa.Column(sa.DateTime, default=datetime.datetime.now)
    updated_at = sa.Column(sa.DateTime, onupdate=datetime.datetime.now)

class SessionPersistence(model_base.BASEV2, HasTimeStamp):
...
class PoolStatistics(model_base.BASEV2, HasTimeStamp):
...
class Vip(model_base.BASEV2, HasTimeStamp, models_v2.HasId, models_v2.HasTenant,
...
class Member(model_base.BASEV2, HasTimeStamp, models_v2.HasId, models_v2.HasTenant,
             models_v2.HasStatusDescription):
...
class Pool(model_base.BASEV2, HasTimeStamp, models_v2.HasId, models_v2.HasTenant,
           models_v2.HasStatusDescription):
    ...
    monitors = orm.relationship("PoolMonitorAssociation", backref="pools",
                                lazy='joined',
                                cascade="all, delete-orphan")
    vip = orm.relationship(Vip, backref='pool',lazy="joined")
    ...
    tenant_vpc = sa.Column(sa.String(255))


class HealthMonitor(model_base.BASEV2, HasTimeStamp, models_v2.HasId, models_v2.HasTenant):
...
class PoolMonitorAssociation(model_base.BASEV2, HasTimeStamp,
                             models_v2.HasStatusDescription):
...
class LoadBalancerPluginDb(loadbalancer.LoadBalancerPluginBase,
    ...
    @notifier.update_or_create_notify
    def update_status(self, context, model, id, status,
                      status_description=None):
        LOG.info(_("Entered the update_status method"))
        with context.session.begin(subtransactions=True):
            LOG.info(_("Started database transaction"))
            if issubclass(model, Vip):
                try:
                    LOG.info(_("Getting VIP from database"))
                ...
                LOG.info(_("Getting other object from database"))
            ...
            LOG.info(_("Updated database object in memory."))
        LOG.info(_("Saved in database."))
    ...
    def create_vip(self, context, vip):
        ...
        with context.session.begin(subtransactions=True):
            start_time = time()
            pool = v.get('pool',None)
            if pool or v['pool_id']:
                pool = pool if pool else self._get_resource(context, Pool, v['pool_id'])
            ...
            LOG.info(_("Took %s secs to create vip in neutron."), (time() - start_time))
            ...
            if v.get('address') is not attributes.ATTR_NOT_SPECIFIED:
                ...
                if port_db:
                    ...
                    vips = self._get_vip_list_by_port_id(context, port_db['id'])
                    vip_with_same_protocol_port = [x for x in vips if x['protocol_port'] == v['protocol_port']]
                    if len(vip_with_same_protocol_port) > 0:
                        raise loadbalancer.DuplicateVIPRequestWithSameIPAndPort(
                            vipname=vip_with_same_protocol_port[0]['name'])
            ...
            if create_new_port:
                try:
                    ...
                    address = self.get_ip_by_port(context, vip_db['port_id'])
                    LOG.info(_("Took %s secs to create port in neutron."), (time() - start_time))
                    start_time = time()

                    self._create_aptr_record(vip_db.name, address, pool['tenant_vpc'])
                    LOG.info(_("Took %s secs to create DNS entries."), (time() - start_time))
                except Exception:
    ...
    @notifier.delete_notify(Vip)
    def delete_vip(self, context, id):
        with context.session.begin(subtransactions=True):
            vip = self._get_resource(context, Vip, id)
            port_id = vip.port.id
            address = self.get_ip_by_port(context, port_id)
        ...
        with context.session.begin(subtransactions=True):
            ...
            if len(vip_list_by_port) == 0:  # this is a Neutron port
                LOG.info("deleting the port %s" % port_id)
                ...
                try :
                    ...
                    # if unable to delete the port, another thread might have deleted it
                    # so ignore it
                    LOG.warn("Unable to delete the port %s" % e)
                    return;

        if len(vip_list_by_port) == 0:
    ...
    @notifier.notify_pool_monitor_association(PoolMonitorAssociation)
    ...
    @log.log
    def create_members_bulk_native(self, context, members):
        self._get_resource(context, Pool, members[0]['member']['pool_id'])
        tenant_id = self._get_tenant_id_for_create(context, members[0]['member']['tenant_id'])
        members_db = []
        try:
            with context.session.begin(subtransactions=True):
                for member in members:
                    ...
                    member_db = Member(id=uuidutils.generate_uuid(),
                                       ...
                                       status=constants.PENDING_CREATE)
                    context.session.add(member_db)
                    members_db.append(self._make_member_dict(member_db))
        except exception.DBDuplicateEntry:
            raise loadbalancer.MemberExists(
                address=v['address'],
                port=v['protocol_port'],
    ...
    @log.log
    def delete_members_bulk_native(self, context, members):
        self._get_resource(context, Pool, members[0]['pool_id'])
        members_db = []
        with context.session.begin(subtransactions=True):
            for member in members:
                self.update_status(context, Member,
                                   member['id'], constants.PENDING_DELETE)
                members_db.append(self._make_member_dict(self._get_resource(context, Member, member['id'])))
        return members_db

    @log.log
    def update_members_bulk_native(self, context, members):
        self._get_resource(context, Pool, members[0]['member']['pool_id'])
        members_db = []
        with context.session.begin(subtransactions=True):
            for v in members:
                new_member = v['member']
                new_member['status'] = constants.PENDING_UPDATE
                existing_member = self._get_resource(context, Member, new_member['id'])
                self.assert_modification_allowed(existing_member)
                existing_member.update(new_member)
                members_db.append(self._make_member_dict(existing_member))
        return members_db
    ...
    def _get_cos_by_tenant_id(self, tenant_id):
        auth_uri = cfg.CONF.keystone_authtoken['auth_protocol'] + "://"+ \
                         cfg.CONF.keystone_authtoken['auth_host'] + ":" + \
                   str(cfg.CONF.keystone_authtoken['auth_port'])+"/v2.0"

        LOG.info('Keystone end point [%s] is used' % auth_uri)
    ...
    def subnet_has_free_ips(self, context, subnet_id):
        """
        This query determines the ip address availability in a subnet by checking ip allocation pool table for the
        allocated range and the ip allocation table for the used ip addresses.
        :param context:
        :param subnet_id:
        :return: True if free ips found. False otherwise
        """

        sql = 'SELECT pool.subnet_id AS subnet_id, total_ips, IFNULL(used, 0) AS used, ' \
              'total_ips - IFNULL(used, 0) AS unused ' \
              'FROM (SELECT subnet_id, sum(1 + INET_ATON(last_ip) - INET_ATON(first_ip)) AS total_ips ' \
                    'FROM ipallocationpools '\
                    'WHERE subnet_id = :subnet_id) pool '\
              'LEFT OUTER JOIN (SELECT subnet_id, count(ip_address) AS used '\
                                'FROM ipallocations '\
                                'WHERE subnet_id = :subnet_id) alloc '\
            'ON pool.subnet_id = alloc.subnet_id '
        result = context.session.execute(sql, {"subnet_id": subnet_id})
        unused_ips = 0
        if result:
            row = result.fetchone()
            if row:
                unused_ips = row['unused']
    ...
    def _create_aptr_record(self, vip_name, ip_address, cos):
        ...
        try:
            ...
            udns_api_version = cfg.CONF.dns['udns_api_version']
            if udns_api_version == "v1":
                dns_vip_resource_name = vip_name + '-' + str_hash
            else:
                dns_vip_resource_name = vip_name + '-vip'
            ...
            if udns_api_version == "v1":
                ...
                return udns_client.create_A_PTR_record_v2(ip_address, dns_vip_resource_name, cos)
    ...
    def _delete_aptr_record(self, cos, ip_address):
        ...
        try:
            ...
            # If the fqdn contains a '.' at the end, remove it.
            if fully_qualified_name[-1] == '.':
            ...
            if fully_qualified_name is not None:
                if udns_api_version == "v1":
                    ...
                    return udns_client.delete_A_PTR_record_v2(ip_address, fully_qualified_name, cos)
            else:
